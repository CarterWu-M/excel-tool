    ...
    public partial class Form1 : Form
    {
        ...
        private void btnFileBrowse_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                {
                    // Get the selected file path
                    gFilePath = openFileDialog.FileName;

                    // Display the file path in the TextBox
                    txtFilePath.Text = gFilePath;
                }
            }
        }
        ...
        private void btnOpen_Click(object sender, EventArgs e)
        {
            ...
            if (IsFileLocked(txtFilePath.Text))
            {
                MessageBox.Show("The file is currently open or locked");
                return;
            }
            excelApp = new Excel.Application();
            workbook = excelApp.Workbooks.Open(txtFilePath.Text);
            ...
            {
                dataTable.Columns.Add((col + 0x40).ToString(), typeof(string));
            }
            ...
            if (null == newWorksheet)
            {
                newWorksheet = workbook.Worksheets.Add(After: workbook.Worksheets[workbook.Worksheets.Count]);
                newWorksheet.Name = "Label_to_Print";
                newWorksheet.PageSetup.Orientation = Excel.XlPageOrientation.xlLandscape;
            }

            if ("Y22-088-USH05" == dataTable.Rows[4 - 1][3 - 1].ToString())
            {
                txtResult.Text = $"{dataTable.Rows[4 - 1][3 - 1].ToString()}: Supported";
                txtResult.BackColor = Color.LightGreen;
                txtResult.Refresh();
            }
            else
            {
                txtResult.Text = $"{dataTable.Rows[4 - 1][3 - 1].ToString()}: Not supported";
